...
class TFExampleCreator(object):
    ...
    def process_nq_entry(self, nq_entry):
        ...
        for nq_feature in nq_features:
            ...
            results.append(tf.train.Example(features=tf.train.Features(feature=features)).SerializeToString())

        return results        
        
    def nq_line_to_nq_instances(self, nq_line):

        nq_entry = nq_line_to_nq_entry(nq_line)
        instances = list(self.process_nq_entry(nq_entry))

        return (nq_entry["has_correct_context"], instances)

    def procss_batch_nq_lines(self, batch_nq_lines, examples_processed, num_examples_with_correct_context, num_spans_to_ids, writer, instance_processed):

        with Pool(2) as p:

            batch_nq_instances = p.map(self.nq_line_to_nq_instances, batch_nq_lines)

        for (has_correct_context, instances) in batch_nq_instances:
        ...
        return examples_processed, num_examples_with_correct_context, instance_processed
    ...
    def process_nq_lines(self, nq_lines, output_tfrecord, max_examples=0, collect_nq_features=False):
        ...
        with tf.io.TFRecordWriter(output_tfrecord) as writer:
            ...
            for idx, nq_line in enumerate(nq_lines):
                
                batch_nq_lines.append(nq_line)

                if len(batch_nq_lines) >= 2 * 100:

                    examples_processed, num_examples_with_correct_context, instance_processed = self.procss_batch_nq_lines(batch_nq_lines, examples_processed, num_examples_with_correct_context, num_spans_to_ids, writer, instance_processed)
                    batch_nq_lines = []

            if len(batch_nq_lines) > 0:
