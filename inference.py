...
flags.DEFINE_string(
    "input_checkpoint_dir", '/kaggle/input/bert-large-epoch-3',#os.path.join(MY_OWN_NQ_DIR, "checkpoints"),
...
bert_tokenizer, bert_for_nq = get_pretrained_model('bert-large-uncased-whole-word-masking-finetuned-squad')
...
os.path.join('/kaggle/input', 'bert-large-epoch-3')
...
checkpoint_path = os.path.join('/kaggle/input', 'bertl-large-2')
...
if ckpt_manager.latest_checkpoint:
    ckpt.restore('/kaggle/input/bertl-large-2/model_ckpt-2')
...
ckpt_manager._checkpoint_prefix = os.path.join(ckpt_manager._directory, "ckpt-2")
print(ckpt_manager._directory)
...
def create_short_answers_from_1_pred(pred):
    ...
    for predicted_short_answer in pred["short_answers"]:
        ...
        if "answer_type_prob_dist" in pred:
            ...
            if pred["answer_type_prob_dist"]["short"] < 0.65 or pred["answer_type_prob_dist"]["unknown"] > 0.65:
